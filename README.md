# CTF Write-ups

Repositório com write-ups de challenges que resolvi em plataformas de segurança ofensiva (TryHackMe, HackTheBox, etc), como parte da minha jornada de estudos em pentest e red team.

## 📋 Índice

| Room/Challenge | Plataforma | Dificuldade | Categoria | Write-up |
|---|---|---|---|---|
| Room 404 | TryHackMe | Easy | Web / Information Disclosure | [Link](./tryhackme/hacker-holidays-room404/README.md) |
| The Concierge Knows Too Much | TryHackMe | Easy | Cloud / AWS Misconfiguration | [Link](./tryhackme/hacker-holidays-concierge-knows-too-much/README.md) |
| Packed Light | TryHackMe | Easy/Medium | Network Forensics / Covert Channel / Malware Reversing | [Link](./tryhackme/hacker-holidays-packed-light/README.md) |

## 🛠️ Sobre

Cada write-up documenta o processo completo de resolução: reconhecimento, descoberta da vulnerabilidade, exploração e aprendizados. As flags são redigidas (`REDACTED`) para não estragar a experiência de quem ainda não completou a room.

## 📫 Contato

- GitHub: [@arthurmaiaz](https://github.com/arthurmaiaz)
- TryHackMe: arthurmaiaz
